...
describe('removeIngredient', () => {
  test('remove main ', () => {
    ...
    const result = constructorItemsReducer(state, removeIngredient('1'));
  ...
  test('состояние, если uuid не найден', () => {
    ...
    const result = constructorItemsReducer(state, removeIngredient('999'));
...
describe('moveIngredient', () => {
  test('поменять местами начинки', () => {
    ...
    expect(result.ingredients.map((i) => i.id)).toEqual(['2', '3', '1']);
  });
